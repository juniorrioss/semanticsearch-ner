...
def main(trial):
    ...
    training_args.output_dir = training_args.output_dir + str(trial._trial_id)
    training_args.run_name = training_args.output_dir
    # ADAMW
    training_args.adam_beta1 = trial.suggest_float("adam_beta1", 1e-3, 1e-1, log=True)
    training_args.adam_beta2 = trial.suggest_float("adam_beta2", 1e-3, 1e-1, log=True)
    ...
    training_args.lr_scheduler_type = trial.suggest_categorical(
        "lr_scheduler_type", ["linear", "cosine_with_restarts", "polynomial"]
    )
    training_args.warmup_ratio = trial.suggest_float("warmup_ratio", 0, 0.1)
    ...
    if training_args.do_eval:
        ...
        trial_metric = metrics["eval_overall_f1"]
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        ...
        study_name="nlp-project",
    )
    study.optimize(main, n_trials=240, n_jobs=20)
